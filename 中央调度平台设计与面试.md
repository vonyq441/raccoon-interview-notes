...
**V2 相对本篇早期示例的关键升级。**Java 在规划前确定性查询候选机器人（当前楼层、心跳新鲜度、占用、能力）与已发布展台，模型据此输出「建议机器人 + 路线」；遇到难以从结构化标签理解的访客偏好，可按需调用受限知识工具查询已发布的展厅业务说明。型号能力、可服务楼层和实时状态仍以 Java 台账/状态快照为准，知识片段只辅助解释。Java 把可修正的校验错误反馈给模型作一次有限修订，人工审核，点击下发时再核实并原子占用机器人。语音则由 Java **严格规则优先**：“下一地点”等完整、明确短句直接形成意图并进入业务校验；复杂、含否定或改线条件的表达才交意图 ChatClient；知识问题才进入当前展台 RAG。以下保留的“只生成展台顺序”代码是较小的解释示例，**不再代表 V2 的完整设计**；V2 的可选知识工具也不能倒推为原上线版本的已证实能力。
...
**先看 V2 的完整目标。**模型不凭一句话猜哪台机器人可用：Java 规划服务先查询机器人台账、由心跳/事件维护的状态快照及展台配置，筛出候选并固定输入版本；楼层未知或状态过期的机器人不得作为“可用”。模型依据真实候选输出 suggestedRobotId 和有序 exhibitCode；若访客偏好需要额外业务背景，可按需调用只读 `searchPlanningContext` 查询已发布说明，不用知识片段推断机器人硬件能力或覆盖 Java 硬约束。Java 校验机器人、必经展台、楼层、知识片段引用和版本，把可修正错误反馈给模型修订；工作人员审核后，下发时重新读取状态并占用建议机器人。原项目这条知识工具调用链是否已上线，仍须以中央 Java 工程和运行记录核实。
...
路线规划、复杂语音分类和问答可以是**共享同一远端 ChatModel 的三个 ChatClient**：分别使用 Java 候选预取与可选规划知识检索、受限意图分类、依据检索证据回答的提示词。前置规则路由是 Java 代码，不是第四个 ChatClient；明确指令不消耗分类模型调用。换模型或调整规则时保持意图枚举和业务接口不变，并重跑否定句、疑问句、口音 ASR 文本及多轮问题测试。移动网关是否兼容 Spring AI 所用协议、实际 `modelId` 和鉴权方式须以提供的 API 文档核实，不能把公开 DeepSeek 地址直接写成移动网关地址。
