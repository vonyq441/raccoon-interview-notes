...
| 模块 | 负责 | 不负责 |
|---|---|---|
| Java中央平台 | 状态管理、任务下发、多机器人调度、路线变更、统一问答 | 关节控制、导航算法 |
...
| ZeroClaw | 执行本机步骤、调用本地技能、回传事件、断线收尾 | 修改全局路线、调度其他机器人 |
| bot_mind | 唤醒、ASR、TTS和语音交互适配 | 全局任务规划 |
...
任务创建后，应把当时使用的`waypointId、explainScript、actionScript`复制到`task_step.snapshot`。如果管理员后来修改了展台配置，已经开始的任务仍按原计划执行，避免路线和讲解内容中途变化。
...
WebSocket不是任务事实来源。平台先把命令写入数据库，再通过WebSocket通知ZeroClaw；机器人重连后可以根据数据库中的待执行命令恢复。
...
`commandId`标识一次命令，`idempotencyKey`防止重复执行，`planVersion`防止机器人继续执行过期路线。
...
接口返回成功只代表ZeroClaw接收了命令。Java只有收到`SUCCEEDED`事件，才认为导航或动作真正完成并推进任务。

### 5.4 心跳和状态同步

Java平台不扫描局域网寻找机器人，而是由机器人侧`bot_mind`主动注册并持续发送心跳。首次启动时，`bot_mind`携带`robotId`、所在楼层、软件版本和能力列表调用注册接口；平台验证机器人已登记且未被禁用后，允许它建立WebSocket连接。
...
平台把最新运行状态写入Redis，并为`robot:online:{robotId}`设置短TTL；每次收到有效心跳就续期。连续多个心跳周期未上报或WebSocket断开时，平台先将机器人标记为疑似离线，超过容错时间后再标记为离线，不再分配新任务。业务数据库中的任务、命令和执行事件仍是恢复与审计的事实来源，Redis只保存易失的最新状态和连接映射。

需要区分“在线”和“可调度”：在线只说明机器人侧服务仍能与平台通信；只有同时满足账号启用、`bot_mind`与导航服务就绪、当前空闲、电量达标且所在区域符合任务要求时，机器人才能进入调度候选集合。机器人可能在线但正在执行任务、电量过低或`g1_base`异常，此时都不能接收新任务。
...
工作人员和规划Agent都可以提出修改，但只能由Java创建新版本：
...
Java只修改尚未开始的步骤，保存新版本并通知ZeroClaw。ZeroClaw完成当前安全步骤后切换到新计划，拒绝旧版本后续命令。高风险调整要求工作人员确认。
...
- ZeroClaw保存最近处理过的`idempotencyKey`，重复命令直接返回原结果；
- Java对`eventId`和`commandId`建立唯一约束，防止重复事件推进两次；
- 命令超时后先查询机器人当前状态，不能立即重复导航；
...
1. **本地安全规则**：ZeroClaw识别“紧急停止”后立即调用停止能力，同时向Java上报；安全停止不能等待网络和大模型。
2. **Java确定性规则**：对“下一个、暂停、继续、重讲、结束”等高频命令做词表和状态匹配。
...
“它有什么优势”必须结合当前展台和上一轮对话判断为知识追问；“不看液冷了，去具身智能展台”则是`PLAN_CHANGE`，不能进入RAG。
...
bot_mind完成ASR
    ↓
ZeroClaw上传taskId、stepId、robotId和文字
    ↓
Java校验当前任务和步骤
    ↓
...
中央问答Agent依据证据生成答案和引用
    ↓
再次校验stepId，避免旧回答串到下一展台
    ↓
ZeroClaw调用bot_mind进行TTS播报
    ↓
...
这样可以隔离SDK阻塞、异常传播、依赖环境和多实例初始化冲突。worker异常时可以独立重启，不拖垮主控制进程。
...
| 命令重复 | ZeroClaw按idempotencyKey返回原结果 |
| 导航超时 | 先查询机器人状态，再决定重试或人工接管 |
| 旧问答返回 | 再次校验stepId，不是当前步骤则丢弃 |
| 计划版本过期 | ZeroClaw拒绝旧planVersion命令 |
| 知识库无可靠证据 | 明确拒答并提示咨询工作人员 |
| 紧急停止 | 机器人本地立即执行，同时上报平台 |
...
模型和知识库集中部署，所有机器人共享同一份展厅知识。机器人侧只保留执行所需能力，减少重复部署和资料不一致。
...
>
> 每台机器人部署ZeroClaw作为单机任务执行器，通过REST和WebSocket接收平台步骤，调用bot_mind、G1ControlServer、g1_base和Unitree SDK完成导航、讲解及动作，并把进度和结果回传平台。语音输入先经过InteractionRouter区分流程指令、知识问答、寒暄和路线调整；知识问题由Java平台统一使用ChatMemory和RAG处理，按当前展台过滤pgvector知识库，再把答案返回机器人播报。
>
...
紧急停止本地优先；常见流程命令由Java规则和状态判断；剩余输入由ChatClient结合当前展台、状态和历史做结构化分类。低置信度追问澄清，模型分类结果还要经过Java校验。
...
可能只是ACK或完成事件丢失，机器人实际已经执行。平台先按commandId查询状态，确认未执行后再重试同一个幂等命令。

#### 9. 路线中途变化怎么办？

Java生成新的planVersion，只修改未开始步骤；ZeroClaw完成当前安全步骤后采用新版本，并拒绝旧版本后续命令。
