...
> 这是一份面向学习和面试的项目文档。主线只有一条：**工作人员说出接待需求，平台选择预配置路线和空闲机器人，机器人按步骤完成导航、讲解、动作与跨楼层交接。**
>
> 项目采用“配置优先、Agent 增强、Java 兜底”的设计。讲解文稿、机器人动作、展台点位和路线都提前配置，中央 Agent 不现场编造讲解内容，也不直接控制电机。
...
系统识别出这是“立即开始的一楼接待任务”，从数据库中找到提前配置好的“一楼领导接待路线”，再选择一台在线、空闲且电量满足要求的 G1 机器人。
...
| 不知道选哪条路线、哪台机器人 | 查询预配置路线，并按规则选择空闲机器人 |
...
数据库保存稳定的点位编码。具体坐标由机器人地图配置维护，中央 Agent 不生成坐标。
...
因此，中央 Agent 的工作是从已有模板中选择，不是自由编写整条路线。
...
routePreference = STANDARD
            ↓
匹配 routeTemplate = F1_LEADER_STANDARD
```

如果没有完全匹配的模板，平台返回可选路线让工作人员确认，而不是让模型编造一条新路线。
...
```text
路线模板                         实际任务
F1_LEADER_STANDARD      →       reception_task T1001
route_step 1            →       task_step T1001-01
route_step 2            →       task_step T1001-02
...
```
...
> Agent 负责听懂，Java 负责做决定和记账，机器人负责安全执行。
...
| 模块 | 负责 | 不负责 |
|---|---|---|
| Spring AI Agent | 理解指令、调用平台工具 | 生成坐标、直接调用电机 |
| Java 调度平台 | 任务、配置、分配、状态和审计 | 局部避障、关节控制 |
| `bot_mind` | 单机交互与上层任务衔接 | 全局多机器人资源分配 |
...
| `tour_point` | 展台与导航点 | `point_code、floor、name、nav_point_code` |
...
| `tour_route_step` | 路线中的有序步骤 | `route_id、sequence、point_code、script_code、wait_after` |

### 10.2 运行数据

| 表 | 作用 | 关键字段 |
|---|---|---|
| `reception_task` | 一场接待任务 | `task_id、route_code、robot_id、status、current_step` |
...
```json
{
  ...
  "type": "NAVIGATE",
  "payload": {
    "navPointCode": "F1_LIQUID_COOLING"
  }
}
```
...
一期不需要为了“架构高级”引入 Kafka、Neo4j、Milvus、强化学习调度或大量微服务。机器人数量少、路线固定时，模块化单体、MySQL 和简单规则调度更容易开发、验证和维护。

问答模块如果需要查询展厅资料，可以后续增加 RAG；它只负责回答临时问题，不负责决定机器人动作，也不影响固定文稿的正常播放。
...
这里的“一体化调度”不是让一个大模型完成所有控制，而是把预配置的路线、文稿、动作和机器人能力连接成统一接待流程。
...
> 基于 Spring AI 设计中央规划 Agent，通过结构化输出和工具调用，将自然语言接待需求映射为预配置路线，并结合机器人在线状态、所在楼层和任务占用完成机器人选择与任务分配。
...
> 完成中央调度平台、规划 Agent 与宇树 G1 动作控制模块集成，打通“接待需求解析—路线匹配—机器人分配—导航讲解与动作执行—状态反馈—跨机器人交接”业务闭环，实现接待任务的统一创建、调度和跟踪，并在甘肃 5G 联合创新中心展厅完成部署与验收。
...
>
> 我主要负责两部分。第一部分是基于 Spring Boot 和 Spring AI 的中央调度平台。Agent 负责识别接待意图并从已有路线中进行匹配，Java 服务再查询机器人在线状态、楼层、电量和任务占用，选择合适机器人并创建任务步骤快照。平台按照状态机依次下发导航、讲解和动作命令，接收机器人事件并更新进度。每个展台讲解后进入等待状态，只有收到“去下一个展台”才继续；跨楼层时由中央平台完成两台机器人的任务交接。
>
...
不是。点位、路线、文稿和动作都由管理员提前配置并审核。大模型只把自然语言需求映射到已有路线，避免现场生成内容不稳定。

#### 2. 既然路线固定，为什么还需要 Agent？

Agent 解决的是自然语言入口。工作人员不必记住路线编码和操作页面，可以直接说出接待需求。真正的路线查询、机器人分配和状态推进仍由 Java 规则完成。
...
展厅机器人数量少、路线固定，规则调度更容易解释和验证。强化学习需要训练环境、奖励设计和安全验证，当前场景没有必要。
...
1. 路线、点位、文稿和动作都是提前配置并审核的。
2. 大模型只解析意图并调用受控工具，Java 负责最终校验和任务状态机。
3. 导航接口返回成功不代表到达，平台必须等待机器人执行事件。
4. 我负责中央平台、规划 Agent 和 G1 动作编排；导航与避障使用团队已有的 `g1_base` 能力。
