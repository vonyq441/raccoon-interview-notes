...
VitePress 在本地开发时读取外层项目根目录的 `面试.md`，单向生成本仓库的 `面试.md` 云端副本和 `docs/interview.md` 构建副本。外层源文档是唯一可编辑版本，站点脚本不会向它写入内容；GitHub Actions 则读取仓库内已提交的 `面试.md`。
...
- `../面试.md`：本地唯一源文件，只读，脚本绝不回写。
- `面试.md`：用于提交到独立文档仓库的云端副本，由同步脚本覆盖。
- `docs/interview.md`：VitePress 构建副本，由同步脚本覆盖，不要手工编辑。
